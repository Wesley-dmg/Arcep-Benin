...
def index(request):
    ...
    sites = (
        Site.objects.select_related("operateur", "localite", "emplacement")
        .prefetch_related("conformite")
        .all()
    )
    ...
    }  # Initialise le dictionnaire pour chaque mois
    ...
    compliance_percentage = (
        (total_conforme / total_sites) * 100 if total_sites > 0 else 0
    )
    ...
    operateur_statistics_counts = [
        sites.filter(operateur=operateur).count() for operateur in operateurs
    ]
    operateur_statistics_non_conform_counts = [
        sites.filter(operateur=operateur, conformite__statut=False).count()
...
def map_view(request):
    ...
    departements = [
        int(dep) for dep in request.GET.getlist("departement") if dep.isdigit()
    ]
        ...
        sites_data.append(
            {
                ...
                "operateur_logo": (
                    site.operateur.logo.url
                    if site.operateur.logo
                    else "/static/assets/img/brand/arcep.png"
                ),
    ...
    context = {
        ...
        "communes": (
            Commune.objects.filter(departement_id__in=departements)
            if departements
            else []
        ),
        "operateurs": Operateur.objects.all(),
...
def get_geojson(request, geojson_type):
    file_path = os.path.join(
        settings.BASE_DIR, "static/geojson", f"BENIN_{geojson_type.upper()}.json"
    )
    try:
...
def operateur_list(request):
    operateurs = Operateur.objects.annotate(
        ...
        conforming_sites=Count(
            "site__conformite", filter=Q(site__conformite__statut=True)
        ),
        non_conforming_sites=Count(
            "site__conformite", filter=Q(site__conformite__statut=False)
        ),
    )
...
def operateur_update(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            handle_message(
                request, f"Erreur lors de la mise à jour de l'opérateur : {e}"
            )
            return redirect("home:operateur_update", pk=pk)

    context = {"operateur": operateur}
...
def operateur_delete(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            handle_message(
                request, f"Erreur lors de la suppression de l'opérateur : {e}"
            )
...
def emplacement_create(request):
    if request.method == "POST":
        try:
            ...
            handle_message(
                request, f"Erreur lors de la création de l'emplacement : {e}"
            )
...
def emplacement_update(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            handle_message(
                request, f"Erreur lors de la mise à jour de l'emplacement : {e}"
            )
...
def emplacement_delete(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            handle_message(
                request, f"Erreur lors de la suppression de l'emplacement : {e}"
            )
...
def commune_update(request, pk):
    ...
    context = {"commune": commune, "departements": departements}
...
def localite_delete(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            handle_message(
                request, f"Erreur lors de la suppression de la localité : {e}"
            )
            return redirect("home:site_create")

    context = {"localite": localite}
    ...
    if ids := request.POST.getlist("ids"):
        if request.method == "POST":
            try:
                ...
                handle_message(
                    request, "Les sites sélectionnés ont été supprimés avec succès."
                )
            except Exception as e:
                handle_message(
                    request, f"Erreur lors de la suppression des sites : {e}"
                )
...
def site_detail(request, pk):
    ...
    conformite = Conformite.objects.filter(
        site=site
    ).first()  # Récupère la première conformité associée au site
        ...
        try:
            ...
            try:
                date_mise_en_service = (
                    datetime.strptime(date_mise_en_service_str, "%Y-%m-%d").date()
                    if date_mise_en_service_str
                    else None
                )
            ...
            for code in technologies_codes:
                try:
                    ...
                    if code in dict(Technologie.TECHNOLOGY_CHOICES):
                        ...
                        logger.error(
                            f"Technologie avec nom '{code}' non trouvée dans les choix."
                        )
                        handle_message(
                            request, f"Technologie '{code}' non trouvée dans les choix."
                        )
                        continue
...
def site_update(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            emplacement = get_object_or_404(
                Emplacement, pk=request.POST.get("emplacement")
            )
            ...
            for code in new_technologies:
                if code not in existing_technologies:
                    if code in dict(Technologie.TECHNOLOGY_CHOICES):
                        technologie, created = Technologie.objects.get_or_create(
                            nom=code
                        )
                        if created:
                            logger.info(f"Technologie ajoutée : {technologie}")
            ...
            Localite.DoesNotExist,
            Operateur.DoesNotExist,
        ) as e:
    ...
    context = {
        ...
        "formatted_date_mise_en_service": (
            site.date_mise_en_service.strftime("%Y-%m-%d")
...
def technologie_list_create(request):
    if request.method == "POST":
        try:
            ...
            # Message de succès et redirection
            handle_message(
                request, "Technologie créée avec succès.", level="success"
            )  # ici level est utilisé correctement
            return redirect("home:technologie_list_create")
        except Exception as e:
            # Message d'erreur en cas de problème
            handle_message(
                request,
                f"Erreur lors de la création de la technologie : {e}",
                level="error",
            )  # ici level est utilisé correctement
            return redirect("home:technologie_list_create")
...
def technologie_update(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            handle_message(
                request,
                f"Erreur lors de la mise à jour de la technologie : {e}",
                tag="error",
            )
...
def technologie_delete(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            handle_message(
                request,
                f"Erreur lors de la suppression de la technologie : {e}",
                level="error",
            )
...
def add_conformite(request, site_id):
    ...
    if request.method == "POST":
        try:
            ...
            try:
                date_inspection = datetime.strptime(
                    date_inspection_str, "%Y-%m-%d"
                ).date()
                date_autorisation = (
                    datetime.strptime(date_autorisation_str, "%Y-%m-%d").date()
                    if date_autorisation_str
                    else None
                )
            except ValueError:
                context = {
...
def update_conformite(request, site_id):
    ...
    context = {"current_site": current_site, "conformite": conformite, "step": 1}
    return render(request, "home/conformite_update_form.html", context)
...
def delete_conformite(request, site_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            current_site.ref_courrier = (
                None  # ou '' si c'est un champ CharField qui n'accepte pas NULL
            )
            ...
            handle_message(
                request,
                "Conformité supprimée et champs du site réinitialisés avec succès.",
            )
            return redirect("home:site_detail", pk=current_site.id)
...
def file_upload_view(request):
    ...
    try:
        ...
        if errors:
            ...
            handle_message(
                request,
                "Certaines lignes n'ont pas pu être importées. Consultez les messages d'erreur.",
            )
        else:
        ...
        logger.exception(
            f"Erreur imprévue lors du téléversement ou du traitement du fichier {uploaded_file.name}: {e}"
        )
